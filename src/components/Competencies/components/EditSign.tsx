...
import { createSign, editSign } from 'api/forms'
...
const EditSign = observer(({ competenceId, sign }: { competenceId: number; sign: Sign }): JSX.Element => {
    const { t } = useTranslation()
    const { competencies } = useStores()
    const { isFetchingCreateEditDeleteSign } = competencies
    const [signType, setSignType] = useState<'single' | 'multi'>(sign.type)
    const [signText, setSignText] = useState<string>(sign.text)
    const [criteria, setCriteria] = useState<ISignSchema[]>([...sign.criteria.map((item) => ({ ...item }))])
    ...
    const onRemoveSign = (signId: number | string) => {
        setCriteria(criteria.filter((item) => item.id !== signId))
    }
            ...
            <Box className="signItemBox">
                <Box sx={{ p: '25px' }}>
                    ...
                    <Button
                        ...
                        variant="contained"
                        onClick={() => {
                            setCriteria([...criteria, { id: shortid.generate(), value: 0, text: '' }])
                        }}
                    >
